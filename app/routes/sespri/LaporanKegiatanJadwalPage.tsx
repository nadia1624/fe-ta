...
export default function LaporanKegiatanJadwalPage() {
  ...
  const filteredTugas = tugasProtokol.filter(tugas => {
    const judul = tugas.agenda?.nama_kegiatan || '';
    const lokasi = tugas.agenda?.lokasi_kegiatan || '';
    const pimpinans = tugas.pimpinans || [];

    const matchesSearch =
      judul.toLowerCase().includes(searchTerm.toLowerCase()) ||
      pimpinans.some((p: any) => 
    ...
    const matchesPimpinan = filterPimpinan === 'all' ||
      pimpinans.some((p: any) => p.nama_pimpinan === filterPimpinan);
  ...
  const statsBerlangsung = tugasProtokol.filter(t => t.status === 'progress' || (t.status === 'pending' && t.laporanKegiatans?.length > 0)).length;
  const statsBelumDimulai = tugasProtokol.filter(t => t.status === 'pending' && (!t.laporanKegiatans || t.laporanKegiatans.length === 0)).length;

  return (
    <div className="space-y-4 md:space-y-6 pb-8">
      {/* Header */}
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-gray-900 tracking-tight">Laporan Kegiatan per Jadwal Pimpinan</h1>
            ...
            <div className="flex flex-col md:flex-row flex-wrap items-stretch md:items-center gap-3">
              ...
              <CustomSelect
                ...
                options={[
                  ...
                  { value: 'progress', label: 'Berlangsung' },
                  { value: 'pending', label: 'Belum Dimulai' }
                ]}
                ...
                <TableBody>
                  {filteredTugas.map((tugas, index) => (
                    <TableRow key={tugas.id_penugasan} className="hover:bg-blue-50/40 transition-colors even:bg-blue-50/60">
                        ...
                        <div className="flex flex-col">
                          {tugas.pimpinans?.map((p: any, idx: number) => (
                            <div key={idx} className="mb-1 last:mb-0">
                              {p.is_representative ? (
                                <>
                                  <span className="text-[13px] font-semibold text-gray-900 leading-tight block">{p.nama_perwakilan}</span>
                                  <span className="text-[10px] text-gray-400 font-medium uppercase tracking-tight">(Wakil {p.nama_pimpinan})</span>
                                </>
                              ) : (
                                <>
                      ...
                      <TableCell className="text-center">
                        <Badge 
                          variant={tugas.status === 'selesai' ? 'success' : (tugas.status === 'progress' || (tugas.status === 'pending' && tugas.laporanKegiatans?.length > 0) ? 'info' : 'warning')} 
                          className="font-bold shadow-sm whitespace-nowrap"
                        >
                          {tugas.status === 'selesai' ? 'Selesai' : (tugas.status === 'progress' || (tugas.status === 'pending' && tugas.laporanKegiatans?.length > 0) ? 'Berlangsung' : 'Belum Dimulai')}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-center">
